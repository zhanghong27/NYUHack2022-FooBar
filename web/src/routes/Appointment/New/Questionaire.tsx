...
const Questionnaire = () => {
  ...
  <FormPage title='Find Doctors' parentPage='/home'>
    ...
    >
      <TextField label='Symptoms' onChange={e => setSymptoms(e.target.value)} value={symptoms}/>
      <FormControl>
        ...
        >
          {doctorTypes.map(doctorType => (
            <MenuItem value={doctorType} key={doctorType}>{doctorType}</MenuItem>
          ))}
        </Select>
      </FormControl>
      <Button onClick={handleGo} disabled={!symptoms || !doctorType}>Go</Button>
    </Box>
  </FormPage>
  )
}
